...
class FiveStageCore : public Core{
	public:
		...
		void step() {
			...
			if (0 == state.ID.nop) {
				...
				if (opcode == "0000011") {// LW <<<<<<<<TODO STALL NEEDED
					...
					if (r1!= 0) {// R0 is always 0
						if(nextState.WB.Wrt_reg_addr.to_ulong() == r1 || nextState.MEM.Wrt_reg_addr.to_ulong() == r1){
					...
					if (func7 == "0000000") {
						...
						if (func3 == "000") {//ADD
							...
							if (r2!= 0) {// R0 is always 0
								if(nextState.WB.Wrt_reg_addr.to_ulong() == r2 || nextState.MEM.Wrt_reg_addr.to_ulong() == r2){
									cout<<"\tHazard detected"<<endl;
									cout<<"\tWB rd: "<<nextState.WB.Wrt_reg_addr.to_ulong()<<endl;
									cout<<"\tMEM rd: "<<nextState.MEM.Wrt_reg_addr.to_ulong()<<endl;
									cout<<"\trs2: "<<r2<<endl;
        	...
        	if (!stall) nextState.EX.nop = state.ID.nop;
			else nextState.EX.nop = true;
			/* --------------------- IF stage --------------------- */
			cout<<"IF stage:"<<"nop --> " << (state.IF.nop? "true": "false") <<endl;
			if(!stall){
				if (0 == state.IF.nop ) {
				bitset<32> Instruction= ext_imem.readInstr(state.IF.PC);            
				cout<<"\tInstruction fetched:\t"<<Instruction<<endl;
				if (Instruction != 0xffffffff) {
					nextState.IF.PC = state.IF.PC.to_ulong() + (stall? 0: 4);
					nextState.IF.nop = false;                               
				} else {// HALTING
					state.IF.nop = true;
